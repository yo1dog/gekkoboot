...
const char *boot_type_names[] = {
  /*BOOT_TYPE_NONE    (0)*/ "NONE",
  /*BOOT_TYPE_DOL     (1)*/ "DOL",
  /*BOOT_TYPE_ONBOARD (2)*/ "ONBOARD"
}
int num_boot_type_names = sizeof(boot_type_names)/sizeof(boot_type_names[0]);

const char *get_boot_type_name(BOOT_TYPE type) {
  if (result < 0 || result >= num_boot_type_names) {
    return "Unknown";
  }
